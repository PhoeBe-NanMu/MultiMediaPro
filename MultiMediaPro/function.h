...
{	
	...
	hDstDIBBits= LocalAlloc(LHND,lWidth * lLineBytes);// 分配临时内存保存行图像
	if (hDstDIBBits == NULL)		
	{		
		return FALSE;									// 分配内存失败
	}		
...
{	
	...
	{			
		...
		lpSrcDIBBits= (char *)lpSrcStartBits + lLineBytes * i ;// 指向倒数第i行，第j个像素的指针								
		lpBits= (char *)lpSrcStartBits + lLineBytes * (lHeight - i - 1);// 指向倒数第i+1行，倒数第j个像素的指针								
